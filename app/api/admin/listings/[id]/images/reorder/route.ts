...
export async function PUT(
  ...
  try {
    ...
    const updates = images.map(async (image: any) => {
      console.log(`Updating image ${image.id}:`, { display_order: image.display_order, is_primary: image.is_primary });
    ...
    await Promise.all(updates);
      ...
      if (primaryImage) {
        ...
        const { data: imageRecord, error: fetchError } = await supabase
          .from('property_images')
          .select('thumbnail_small_url, thumbnail_medium_url, thumbnail_large_url')
