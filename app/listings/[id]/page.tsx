        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-2 space-y-4 md:space-y-6">
            ...
            <Card className="p-4 md:p-6">
              <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-3 md:mb-4">About This Property</h2>
              <p className="text-sm md:text-base text-gray-700 whitespace-pre-line">{listing.description}</p>
            </Card>

            {/* Property Features */}
            <Card className="p-4 md:p-6">
              <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-3 md:mb-4">Features & Amenities</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                {listing.bedrooms && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-700">
                      <strong className="text-gray-900">{listing.bedrooms}</strong> Bedrooms
                    </span>
                  </div>
                )}
                {listing.bathrooms && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-700">
                      <strong className="text-gray-900">{listing.bathrooms}</strong> Bathrooms
                    </span>
                  </div>
                )}
                {listing.square_feet && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-700">
                      <strong className="text-gray-900">{listing.square_feet.toLocaleString()}</strong> sqft
                    </span>
                  </div>
                )}
                {listing.lot_size && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-700">
                      <strong className="text-gray-900">{listing.lot_size}</strong> {listing.lot_size_unit || "acres"} lot
                    </span>
                  </div>
                )}
                {listing.year_built && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-700">
                      Built in <strong className="text-gray-900">{listing.year_built}</strong>
                    </span>
                  </div>
                )}
                {listing.garage_spaces && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-700">
                      <strong className="text-gray-900">{listing.garage_spaces}</strong> Car Garage
                    </span>
                  </div>
                )}
              </div>
              ...
              {listing.features && Object.keys(listing.features).length > 0 && (
                <div className="mt-6">
                  <h3 className="font-semibold text-gray-800 mb-3">Additional Features</h3>
          ...
          <div className="lg:col-span-1">
            <Card className="p-5 md:p-6 lg:sticky lg:top-8">
              ...
              <div className="border-t border-gray-200 pt-6 mb-6">
                ...
                <div className="space-y-2.5 text-sm">
                  <div className="flex justify-between gap-2">
                    ...
                    <span className="font-medium text-gray-900 capitalize text-right">{listing.property_type.replace(/_/g, " ")}</span>
                  </div>
                  <div className="flex justify-between gap-2">
                    <span className="text-gray-600">Location:</span>
                    <span className="font-medium text-gray-900 text-right">{listing.city}, {listing.state}</span>
                  </div>
                  <div className="flex justify-between gap-2">
                    <span className="text-gray-600 flex-shrink-0">Address:</span>
                    <span className="font-medium text-gray-900 text-right break-words">{listing.street_address}</span>
                  </div>
                  {listing.hoa_fees && (
                    <div className="flex justify-between gap-2">
                      <span className="text-gray-600">HOA Fees:</span>
                      <span className="font-medium text-gray-900">${listing.hoa_fees.toLocaleString()}/mo</span>
                    </div>
                  )}
                  {listing.property_tax && (
                    <div className="flex justify-between gap-2">
                      <span className="text-gray-600 flex-shrink-0">Property Tax:</span>
                      <span className="font-medium text-gray-900">${listing.property_tax.toLocaleString()}/yr</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="border-t border-gray-200 pt-6">
                ...
                <div className="space-y-3 text-sm">
                  <div>
                    ...
                    <p className="font-medium text-gray-900 break-words">{listing.contact_name}</p>
                  </div>
                  <div>
                ...
                <Button
                  ...
                  onClick={() => window.location.href = `mailto:${listing.contact_email}?subject=Inquiry about ${listing.title}`}
                >
                  Contact Seller
                </Button>
              </div>
              ...
              <div className="border-t border-gray-200 pt-6 mt-6 text-xs text-gray-500">
                ...
                {listing.reviewed_at && (
                  <p>Verified on {new Date(listing.reviewed_at).toLocaleDateString()}</p>
                )}
              </div>
            </Card>
          </div>
